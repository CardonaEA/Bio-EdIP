...
col6 = handles.T;
% save img names
col0 = handles.listfiles; 
% variable names
varnames = {'Image','Cell_Free_Region_percentage','Cell_Free_Region_pixels','Cell_Covered_Region_percentage','Cell_Covered_Region_pixels','Blocks','Threshold'};
% Cat all data
d = table(col0(:), col1(:), col2(:), col3(:), col4(:), col5(:), col6(:), 'VariableNames', varnames);
% get dir to save    
...
if folder ~= 0
    ...
    for s = 1:handles.numfiles
        imwrite(ImSave{s},[folder sep filename{s}(1:end-4) '_Outline' '.tif'],'compression','none')
        imwrite(ImSave1{s},[folder sep filename{s}(1:end-4) '_Overlay' '.tif'],'compression','none')
        imwrite(ImSaveB{s},[folder sep filename{s}(1:end-4) '_Binary' '.tif'],'compression','none')
        %Waitbar
        waitbar(s / handles.numfiles)
        drawnow
    end
    % Save data 
    writetable(d, [folder sep 'Analysis.csv'])
    tocite = table({'How to cite';'Cardona A, Ariza-Jiménez L, Uribe D, Arroyave JC, Galeano J, Cortés-Mancera FM. Bio-EdIP: An Automatic Approach for In vitro Cell Confluence Images Quantification. Comput Methods Programs Biomed 2017;145:23-33. doi:10.1016/j.cmpb.2017.03.026'}); 
    writetable(tocite,[folder sep 'ToCite.txt'],'FileType','text','Delimiter','tab','WriteVariableNames',false,'QuoteStrings',false)
    % Saved finished
